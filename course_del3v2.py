...
def webmnt():  # Gets webdav path from user.
    ...
    while not os.path.isdir(wmount):
        ...
        wmount = input('Path to webdav mount: ')
...
def feedlocal():  # Gets feed file path from user.
    ...
    while not os.path.isfile(feedloc):
        ...
        feedloc = input('Provide path to feedfile: ')
...
def loglocal():  # Gets location user wants logs created in. Logs are created with current
    ...
    while not os.path.isdir(logloc):
        ...
        logloc = input('Please provide directory path for logging: ')
        print(logloc)
...
def confirm_paths(webdav, feed, rmlog, errlog):  # Final check to confirm paths. Users can go back
    ...
    print('4. Error Log: %s \n' % errlog)
    conf = input('Are these correct? (Y/N): ')
    if conf == 'Y' or conf == 'y':
        removal(webdav, feed, rmlog, errlog)
    else:
        cornum = input('Which one is incorrect (1/2/3): ')
        if cornum == '1':
            webdav = webmnt()
...
def get_size(rmdir):
    total_size = 0
    for dirpath, dirnames, filenames in os.walk(rmdir):
        for f in filenames:
...
def safety_dance(webdav, dirt, rmlog, errlog):  # Check to ensure we don't attempt to delete /courses
    rmdir = os.path.join(webdav, dirt)  # This is checked 2 ways. Comparing paths and
    if os.path.isdir(rmdir):  # then checking for an empty line in the feed file.
        if os.path.normpath(rmdir) == os.path.normpath(webdav) or len(dirt.rstrip()) == 0:  # If anyone comes up with
...
def removal(webdav, feed, rmlog, errlog):  # open the feed, check the line for a blank or missing directory,
    ...
    with open(feed, 'r') as fd:  # We also track progress through current/fleng()
        for line in fd:
            current += 1
            line = line.rstrip('\n')
            rem_logs(rmlog, 'Processing: %s %d/%d \n' % (line, current, feedlen))
            rem_logs(errlog, 'Processing: %s %d/%d \n' % (line, current, feedlen))
            rmdir = safety_dance(webdav, line, rmlog, errlog)
            if rmdir == 'no':
                pass
            elif get_size(rmdir) == 0:
                try:
                    # shutil.rmtree(rmdir)
                    rem_logs(rmlog, 'Successfully Deleted - Small Directory: %s \n' % rmdir)
                except Exception as err1:
                    rem_logs(errlog, 'Exception while processing: %s \n %s \n' % (rmdir, err1))
            else:
                for dirt, file, path in os.walk(rmdir, topdown=False):
                    try:  # try to remove - get exception or delete.
                        ...
                        time.sleep(300)
                        rem_logs(errlog,
                                 'Exception while processing: %s \n %s \n Pausing for 5 min until webdav folder is '
                                 'available. \n' % (
                                     dirt, err1))
